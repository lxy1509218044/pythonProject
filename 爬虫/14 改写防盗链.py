...
class Spider(object):
    def __init__(self):
        # 视频原始url
        self.url = "https://www.pearvideo.com/video_1764559"
        # split()：拆分字符串。通过指定分隔符对字符串进行切片，并返回分割后的字符串列表（list）
        contID = self.url.split("_")[1]
        # 拿到抓包工具里的Request URL,然后对url进行修复得到真正的直链视频url
        self.url1 = f"https://www.pearvideo.com/videoStatus.jsp?contId={contID}&mrd=0.19786460056378763"
    # 定义常用变量,比如url或计数变量等

    def get_html(self,url,url1):
        videoStatus = url1
        head = {
            # UA
    ...
    def parse_html(self,contID,resp):
        dict = resp.json()
        # print(dic)
